...
#   2. The value of env $CHART_PKG should be the path of `private-cloud/.chart-packages`.
#   3. The directory of env $CHARTS_INDEX should be create first under the root path of repo charts.

set -ex

BINDIR=`dirname "$0"`
# REPO_PATH is the path of private-cloud
...
# CHART_PKG is the path of private-cloud chart package which is packaged in release.sh
